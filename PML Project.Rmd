...
This document describes my apporoach to building a predictive model for the Weight Lifting Exercises Dataset.
The objective is to fit a predictive model to the provided data in order to predict the kind of weightlifting that was performed.
It turns out that we can fit a random forest model which predicts very well.

### Getting and cleaning Data

First let's get the data for training and validating the model. 
Inspect data by looking at the output of `str(pml.data)`.
Some variables are set to factor, but when we look at the levels, they appear to be numeric.
Closer inspection reveals that:

* Some variables are useless (e.g. `levels(pml.data$kurtosis_yaw_belt)` -> "#DIV/0!")
* Some variables have a wide range of numeric values but also a value "#DIV/0!".
...
There are many variables which have alot of NA values.
This makes them less suitable for prediction.
Initially, let`s try to build a model using only variables that have no NA values.
...
The 60/40 split has been chosen for training/testing.
...
Before to fit the model, check if the training set contains numeric variables which have no or close to zero variance.

```{r cache=TRUE}
nearZeroVar(pml.data.train[, sapply(pml.data.train, is.numeric)])
```

There are no numeric variables with near zero variance, so I cannot reduce the number of variables this way.
Therefore I use all the remaining variables in the training data set.
...
So the model succeeded quite well in predicting the activities on the training data set.


### Predict new data

Use the model to predict the verification data.
...
I used `ncol pml.data.train` variables.
Looking at the variable importances in the model, I see a rather quick drop off.
...
With the new model, let`s predict again the test data:

```{r cache=TRUE}
confusionMatrix(new.test.data$classe, predict(modelFit.reduced, new.test.data))
```

We get about as good results as with the initial model and training this model took ~30 minutes instead of ~90 minutes for the initial model.